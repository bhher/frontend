...
export default function App() {
  ...
  //  실제 DOM 요소(?.current)의 상단이 화면 상단에 맞춰 부드럽게(behavior: 'smooth',
  // block: 'start') 보이도록 스크롤합니다.
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      sections.forEach((section, index) => {
        const top = section.offsetTop; //현재순회줃인 섹션의 시작
        const bottom = top + section.offsetHeight;
        //crollPosition (감지위치)가 현재 순회중인 센션의 시작과 끝 사이에 있는경우
  ...
  //컴포넌트 화면에 나타난후 , 사용자가 스크롤 할 때 마다 모든 .section 요소를 검사하여
  //화면에 상단 1/3 지점에 걸여 있는 세션이 무엇인지 확인하고, 그섹션의 인덱스를
  //activeSetion에 상태 저장해
  //이 activeSetion상태를 사용하여 내비게이션 메뉴의 스타일을 동적으로 변경할 수 있다.
